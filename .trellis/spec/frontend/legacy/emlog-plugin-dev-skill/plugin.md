...
plog支持插件机制，这样使得开发者可以方便地向系统中添加自己需要的功能。

## 实现原理

在plog整个运行过程中我们设定了一些动作事件，遇到这些事件时plog会自动的调用插件绑定到该事件的上的所有插件函数，从而实现插件的功能。

## 挂载点函数：doAction

doAction 函数内置于plog核心代码中，就是所谓的插件挂载点。
...
Plugin Name: 小贴士
Version: 3.0
Plugin URL:https://www.plog.net/plugin/detail/xxx
...
:::tip
其中 Plugin URL 和 Author URL 请使用官网 plog.net 的应用链接和作者页，其他非官网链接不会在后台插件列表展示超链接。
:::

### 事件回调

在plog后台的插件管理页面，用户可以开启插件、关闭插件、删除插件，还可以更新插件。这些操作有的会触发对应的回调函数。
...
:::danger
插件不得修改plog核心数据库表及字段，包括向核心表增加字段。特别是增加没有默认值的字段。
:::
...
plog系统自带的tips插件，也是官方提供的插件演示demo，可以基于该插件进行修改来开发自己的插件。
tips插件所在目录: content/plugins/tips

## 开源发布

如果你开发的插件在 Github 开源，那请为仓库添加topic： [plog-plugin](https://github.com/topics/plog-plugin)
...
2. 发布：登录官网plog.net - 我的 - 应用开发 - 发布插件，按照提示填写必要的信息即可发布，等待审核。
