...
plog可以方便的更换模板，模板文件位于 content\templates 目录下，每个模板都是一个单独的文件夹，文件夹以模板的英文别名命名。通过应用商店以及后台上传安装的模板都保存在这个目录下。
...
plog未采用任何其他第三方的模板引擎，直接使用PHP原生的语法标记来内嵌HTML生成动态页面。这样不但降低了开发者的学习负担，也大大提高了页面加载和渲染效率。
...
Description:plog的默认模板
Author:plog
Author Url:https://www.plog.net/author/index/577
*/
```

:::tip
其中 Template URL 和 Author URL 请使用官网 plog.net 的应用链接和作者页，其他非官网链接不会在后台展示超链接。
:::
...
| $blogname         | 变量 | 站点标题                                                                                                     |
| $bloginfo         | 变量 | 站点副标题                                                                                                   |
| BLOG_URL          | 常量 | 站点首页的URL，输出形如https://plog.net/                                                                    |
| TEMPLATE_URL      | 常量 | 模板文件夹的URL，用于加载模板内的css、js及其他内容，输出形如http://plog.net/blog/content/templates/default/ |
...
| Option::PLOG_VERSION | 常量 | 当前plog版本号        |
...
如在自定义函数内调用plog缓存时，假设读取user缓存信息，则形如： global $CACHE; $user_cache = $CACHE→readCache('user');
...
在plog后台的外观-模板管理页面，用户可以开启、删除、更新模板。这些操作会触发对应的回调函数。
...
function optionIconFont() {
    echo sprintf('<link rel="stylesheet" href="%s">', 'https://cdn.bootcdn.net/ajax/libs/remixicon/3.5.0/remixicon.min.css?ver=' . Option::PLOG_VERSION_TIMESTAMP);
}
...
| 变量、常量            | 类型 | 说明                                                                 |
| --------------------- | ---- | -------------------------------------------------------------------- |
| Option::PLOG_VERSION | 常量 | 获得当前plog版本号                                                  |
...
| TEMPLATE_PATH         | 常量 | 当前模板的绝对路径 eg:/www/plog/content/templates/default/          |
...
搜索路由：https://yoursite_domain/?keyword=plog&sid=1
...
plog系统自带的默认主题就是最好的演示demo，可以基于该主题进行修改来开发自己的主题，或者参考该主题的部分元素和文件。
默认模板所在目录: content/templates/default

## 开源发布

如果你开发的模板在 Github 开源，那请为仓库添加topic： [plog-template](https://github.com/topics/plog-template)
...
2. 发布：登录官网plog.net - 我的 - 应用开发 - 发布模板，按照提示填写必要的信息即可发布，等待审核。
