...
if ($action === 'backup') {
    ...
    $filename = 'plog_' . Option::PLOG_VERSION . '_' . date('Ymd_His');
    ...
    $tempFile = tempnam(sys_get_temp_dir(), 'plog_backup_');
    $fp = fopen($tempFile, 'w');
    ...
    // 写入备份文件头信息
    fwrite($fp, '#version:plog ' . Option::PLOG_VERSION . "\n");
...
function checkSqlFileInfo($sqlfile)
{
    ...
    fclose($fp);
    if (empty($dumpinfo)) {
        emMsg('该文件不是plog的数据备份文件');
    }

    if (preg_match("/pro\s\d+\.\d+\.\d+/", $dumpinfo[0], $matches)) {
        $v = $matches[0];
        if ($v !== Option::PLOG_VERSION) {
            emMsg('不是当前版本生成的数据备份，请安装 plog ' . $v . ' 导入。');
        }
    } else {
        emMsg('该文件不是 plog pro 的数据备份文件');
    }
