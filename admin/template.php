...
if ($action === 'check_update') {
    ...
    $emcurl->request('https://store.plog.net/template/upgrade');
    ...
    if ($ret['code'] === MSGCODE_EMKEY_INVALID) {
        Output::error(_lang('plog_not_registered'));
    }
...
if ($action === 'upgrade') {
    ...
    if (!Register::isRegLocal()) {
        Output::error(_lang('plog_not_registered'), 200);
    }

    $temp_file = emFetchFile('https://www.plog.net/template/down/' . $alias);
    if (!$temp_file) {
