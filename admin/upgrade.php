...
if ($action === 'check_update') {
    ...
    $emcurl->setPost([
        ...
        'version'   => Option::PLOG_VERSION,
        'timestamp' => Option::PLOG_VERSION_TIMESTAMP,
    ]);

    $emcurl->request('https://store.plog.net/service/upgrade');
...
if ($action === 'update' && User::isAdmin()) {
    ...
    foreach ($sql as $value) {
        ...
        if (!empty($value) && $value[0] == '#') {
            ...
            if (version_compare(Option::PLOG_VERSION, $ver) > 0) {
                break;
            }
        }
        if (!$value || $value[0] == '#') {
