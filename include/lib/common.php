...
function emAutoload($class)
{
    ...
    if (file_exists(PLOG_ROOT . '/include/model/' . $class . '.php')) {
        require_once(PLOG_ROOT . '/include/model/' . $class . '.php');
    } elseif (file_exists(PLOG_ROOT . '/include/lib/' . $class . '.php')) {
        require_once(PLOG_ROOT . '/include/lib/' . $class . '.php');
    } elseif (file_exists(PLOG_ROOT . '/include/controller/' . $class . '.php')) {
        require_once(PLOG_ROOT . '/include/controller/' . $class . '.php');
    } elseif (file_exists(PLOG_ROOT . '/include/service/' . $class . '.php')) {
        require_once(PLOG_ROOT . '/include/service/' . $class . '.php');
    }
}
...
function realUrl()
{
    ...
    $plog_path = PLOG_ROOT . DIRECTORY_SEPARATOR;
    ...
    while ($current_deep < $max_deep) {
        ...
        if (substr($plog_path, strlen($this_match) * (-1)) === $this_match) {
...
{
    if (is_string($plugin) && preg_match("/^[\w\-\/]+\.php$/", $plugin) && file_exists(PLOG_ROOT . '/content/plugins/' . $plugin)) {
        return true;
    }
...
function emZip($orig_fname, $content)
{
    ...
    $zip = new ZipArchive();
    $tempzip = PLOG_ROOT . '/content/cache/emtemp.zip';
...
{
    $temp_file = tempnam(PLOG_ROOT . '/content/cache/', 'tmp_');
    ...
    return [
        'http' => [
            ...
            'header'  => "Referer: " . BLOG_URL . "\r\n"
                . "Emkey: " . $emkey . "\r\n"
                . "User-Agent: plog " . Option::PLOG_VERSION . "\r\n",
        ],
        "ssl"  => [
            "verify_peer"      => false,
